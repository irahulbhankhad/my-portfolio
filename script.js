...
if (heroNameWrap && heroBeeOrbits.length && !reduceMotionPref.matches) {
...
function initHeroBeeParallaxAndRandomOrbit(wrap, orbits) {
    ...
    const profiles = [
        { x: [-14, -4], y: [-14, -4], z: [-24, -10], sx: [-8, -1], sy: [-3, 4], dur: [5.2, 6.5], trail: [0.5, 0.7] },
        { x: [0, 10], y: [-4, 8], z: [-4, 12], sx: [0, 8], sy: [-5, 3], dur: [6.8, 8.6], trail: [0.62, 0.86] },
        { x: [-18, -8], y: [-20, -10], z: [-28, -12], sx: [-5, 4], sy: [1, 8], dur: [4.8, 5.9], trail: [0.48, 0.68] }
    ];

    const applyOrbitMutation = (orbit, index) => {
        ...
        orbit.style.setProperty('--orbit-x', `${(rand(profile.x[0], profile.x[1]) * mobileScale).toFixed(2)}px`);
        orbit.style.setProperty('--orbit-y', `${(rand(profile.y[0], profile.y[1]) * mobileScale).toFixed(2)}px`);
        orbit.style.setProperty('--orbit-z', `${rand(profile.z[0], profile.z[1]).toFixed(2)}px`);
        orbit.style.setProperty('--orbit-shift-x', `${(rand(profile.sx[0], profile.sx[1]) * mobileScale).toFixed(2)}px`);
        orbit.style.setProperty('--orbit-shift-y', `${(rand(profile.sy[0], profile.sy[1]) * mobileScale).toFixed(2)}px`);
        orbit.style.setProperty('--trail-opacity', `${rand(profile.trail[0], profile.trail[1]).toFixed(2)}`);
        orbit.style.setProperty('--start-angle', `${rand(0, 360).toFixed(2)}deg`);
        orbit.style.setProperty('--orbit-duration', `${duration.toFixed(2)}s`);
        orbit.style.setProperty('--orbit-delay', `${(-rand(0.1, 1) * duration).toFixed(2)}s`);
        orbit.style.setProperty('--bee-wobble-duration', `${rand(1.2, 2.15).toFixed(2)}s`);
        orbit.style.animationDirection = Math.random() > 0.56 ? 'reverse' : 'normal';
    };

    orbits.forEach((orbit, index) => {
        applyOrbitMutation(orbit, index);
        const mutate = () => {
            applyOrbitMutation(orbit, index);
            setTimeout(mutate, rand(1700, 3600));
        };
        setTimeout(mutate, rand(900, 1800));
    });
